...
// ===== AJUDA =====
const HELPS={
  painel:{t:'📊 Como funciona o Painel',b:`<strong>Saldo Disponível</strong>Entradas do mês menos o que você já pagou (saídas avulsas + parcelas marcadas como pagas).<strong>Breakdown (3 boxes)</strong>Entradas / Saídas avulsas / Parcelas pagas — separadas para você entender de onde vem cada valor.<strong>Mês / Trimestre / Ano</strong>Filtra o período dos valores mostrados.<strong>Mês anterior</strong>Mostra o saldo do mês passado para comparar.`},
...
window.addEventListener('DOMContentLoaded',()=>{
  // App ready - show splash briefly then load
  const t=Date.now();
  function launch(){
  ...
  // Min 300ms splash so it doesn't flash, max 800ms
  setTimeout(launch, Math.max(0, 300-elapsed));
});
...
function renderDash(){
  ...
  document.getElementById('qPar').textContent=DB.fmtShort(curPeriod==='mes'?DB.parcelasDoMes(y,m).reduce((s,x)=>s+x.par.valorParcela,0):r.totalParcelasVenc);
  ...
  const rPrev=DB.resumoMes(py,pm);
