...
function renderDash(){
  ...
  // Update period label on hero
  const periodoLabel={mes:`${MESES[m]} ${y}`,trimestre:'Último trimestre',ano:`Ano ${y}`}[curPeriod];
  const plEl=document.getElementById('periodoLabel');
  ...
  // Show clear breakdown: what goes INTO the saldo calculation
  ...
  // Breakdown boxes — label them clearly
  ...
  // Parcelas: show pending+overdue for current month (what still needs to be paid)
  const parMesAtual=DB.parcelasDoMes(y,m);
  const parVencMes=parMesAtual.reduce((s,x)=>s+x.par.valorParcela,0);
  const parPendentesMes=parMesAtual.filter(x=>x.status!=='pago').reduce((s,x)=>s+x.par.valorParcela,0);
  document.getElementById('qPar').textContent=DB.fmtShort(curPeriod==='mes'?parVencMes:r.totalParcelasVenc);

  // Show pending alert on parcelas card
  const parCard=document.getElementById('qParCard');
  if(parCard&&curPeriod==='mes'&&parPendentesMes>0){
    const sub=parCard.querySelector('.qv-sub');
    if(sub) sub.textContent=`${DB.fmtShort(parPendentesMes)} pendente`;
  }

  // Previsão próximo mês
  const pk=prevMonthKey(1),pd=DB.getPrev(pk);
  const ps=pd.entradas.reduce((s,i)=>s+i.valor,0)-pd.saidas.reduce((s,i)=>s+i.valor,0);
  document.getElementById('qPrev').textContent=(pd.entradas.length||pd.saidas.length)?DB.fmtShort(ps):'Próx. mês';

  // Mês anterior
  const prev=new Date(y,m-1,1),py=prev.getFullYear(),pm2=prev.getMonth();
  const rPrev=DB.resumoMes(py,pm2);
  const maEl=document.getElementById('mesAnteriorVal');
  if(maEl){
    maEl.textContent=DB.fmt(rPrev.saldoReal);
    maEl.style.color=rPrev.saldoReal>=0?'var(--green)':'var(--red)';
    document.getElementById('mesAnteriorTend').textContent=rPrev.saldoReal>=0?'😊':'😟';
    // Show mes anterior label
    const maLbl=document.getElementById('mesAnteriorLbl');
    if(maLbl) maLbl.textContent=`${MESES[pm2]} ${py}`;
  }

  // Saldo acumulado TOTAL (todas as entradas - todas as saídas históricas)
  const allTx=DB.getTx();
  const totalEntHist=allTx.filter(t=>t.tipo==='entrada').reduce((s,t)=>s+t.valor,0);
  const totalSaiHist=allTx.filter(t=>t.tipo==='saida'&&!t._parId).reduce((s,t)=>s+t.valor,0);
  const totalParPagasHist=DB.getPar().reduce((s,p)=>{
    return s+p.pagamentos.filter(x=>x?.status==='pago').reduce((ps,x,i)=>ps+(x.valorPago||p.valorParcela),0);
  },0);
  const saldoHistorico=totalEntHist-totalSaiHist-totalParPagasHist;
  const shEl=document.getElementById('saldoHistorico');
  if(shEl){
    shEl.textContent=DB.fmt(saldoHistorico);
    shEl.style.color=saldoHistorico>=0?'var(--green)':'var(--red)';
  }
  ...
  renderRecentTxDash();
  genSugestoes(r);
  renderCalcExplain(r, periodoLabel);
}

function renderCalcExplain(r, periodo){
  const rows=document.getElementById('ceRows');
  const tot=document.getElementById('ceTotal');
  if(!rows||!tot) return;

  const itens=[
    {lbl:`(+) Entradas — ${periodo}`, val:r.entradas, cls:'pos'},
    {lbl:`(−) Saídas avulsas pagas`, val:-r.saidasAvulsas, cls:'neg'},
    {lbl:`(−) Parcelas pagas no período`, val:-r.parcelasPagas, cls:'neg'},
  ];

  if((r.parcelasPendentes||0)>0){
    itens.push({lbl:`⏳ Parcelas pendentes (não deduzidas)`, val:r.parcelasPendentes, cls:'neu'});
  }
  if((r.parcelasAtrasadas||0)>0){
    itens.push({lbl:`⚠️ Parcelas atrasadas (não deduzidas)`, val:r.parcelasAtrasadas, cls:'neg'});
  }

  rows.innerHTML=itens.map(it=>`
    <div class="ce-row">
      <span class="ce-lbl">${it.lbl}</span>
      <span class="ce-val ${it.cls}">${it.val>=0?'+':''}${DB.fmt(Math.abs(it.val))}</span>
    </div>
  `).join('');

  tot.textContent=DB.fmt(r.saldoReal);
  tot.style.color=r.saldoReal>=0?'var(--green)':'var(--red)';
}
...
function parCardHTML(p,ty,tm){
  ...
  for(let i=0;i<p.nParcelas;i++){
    ...
    const dueLabel=`Parcela ${i+1} — vence ${String(due.getDate()).padStart(2,'0')}/${MESES[due.getMonth()].slice(0,3)}/${due.getFullYear()}: ${st}`;
...
function abrirPagModal(parId,idx){
  ...
  if(cur==='pago'){
    ...
    renderParcelas(); renderDash(); if(curPage==='saidas') renderSaidas(); return;
  }
  if(cur==='atrasado'){
    par.pagamentos[idx]={status:'pendente',dataPagamento:null,valorPago:null};
    DB.updatePar(parId,{pagamentos:par.pagamentos});
    showToast(`Parcela ${idx+1} pendente`); renderParcelas(); renderDash(); return;
  }
  // pendente ou futuro → abre modal de pagamento
  ...
  document.getElementById('pagModalTitle').textContent=`✅ Parcela ${idx+1}/${par.nParcelas} — ${par.descricao}`;
  // Adiciona info da parcela
  const infoEl=document.querySelector('.pag-par-info');
  if(infoEl){ infoEl.querySelector('.pag-desc').textContent=`Vence: ${String(due.getDate()).padStart(2,'0')}/${MESES[due.getMonth()]} ${due.getFullYear()}`; infoEl.querySelector('.pag-val').textContent=DB.fmt(par.valorParcela); }
...
function renderFamilia(){
  ...
  const all=[{id:'__me__',nome:myNome,mesEnt:r.entradas,mesSai:r.totalSaidasPagas,saldo:r.saldoReal,isMe:true},...members];
