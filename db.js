...
// ===== PREVISÃO =====
DB.getPrev = function(monthKey){ return this.getObj('prev_'+monthKey,{entradas:[],saidas:[]}); };
DB.savePrev = function(monthKey,data){ this.setObj('prev_'+monthKey,data); };

// ===== FAMÍLIA =====
DB.getFamMembers = function(){ return this.get('fam_members'); };
DB.saveFamMembers = function(a){ this.set('fam_members',a); };
DB.addFamMember = function(m){ const a=this.getFamMembers(); m.id=Date.now()+Math.random(); m.importedAt=new Date().toISOString(); a.push(m); this.saveFamMembers(a); return m; };
DB.removeFamMember = function(id){ this.saveFamMembers(this.getFamMembers().filter(m=>m.id!=id)); };

DB.exportMyData = function(nome){
  const tx=this.getTx(), pars=this.getPar();
  const now=new Date(), y=now.getFullYear(), mo=now.getMonth();
  const mesEnt=tx.filter(t=>{ const d=new Date(t.data+'T00:00:00'); return t.tipo==='entrada'&&d.getFullYear()===y&&d.getMonth()===mo; }).reduce((s,t)=>s+t.valor,0);
  const mesSai=tx.filter(t=>{ const d=new Date(t.data+'T00:00:00'); return t.tipo==='saida'&&d.getFullYear()===y&&d.getMonth()===mo; }).reduce((s,t)=>s+t.valor,0);
  const parMes=this.parMensalAtiva();
  const payload={ v:2, nome:nome||'Membro', mesEnt, mesSai:mesSai+parMes, parMes, saldo:mesEnt-(mesSai+parMes), exportedAt:new Date().toISOString() };
  return btoa(unescape(encodeURIComponent(JSON.stringify(payload))));
};

DB.importFamData = function(code){
  try{
    const json=decodeURIComponent(escape(atob(code.trim())));
    const data=JSON.parse(json);
    if(!data.nome) throw new Error('invalid');
    this.addFamMember(data); return data;
  }catch(e){ return null; }
};
